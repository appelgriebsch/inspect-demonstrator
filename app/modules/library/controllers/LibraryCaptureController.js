(function(angular) {
  ...
  function LibraryCaptureController($scope, $state, $q, DocumentCaptureService, LibraryDataService) {
    ...
    webViewer.addEventListener('ipc-message', (evt, args) => {
      ...
      template.datePublished = meta.publicationDate ? (meta.publicationDate.indexOf('Z') > 0 ? meta.publicationDate : (meta.publicationDate.indexOf('+') > 0 ? meta.publicationDate : (meta.publicationDate.length > 0 ? `${meta.publicationDate}${dateTimeOffset}` : ''))) : '';
