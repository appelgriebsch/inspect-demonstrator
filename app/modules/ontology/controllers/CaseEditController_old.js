(function(angular, vis) {
  ...
  function CaseEditController($scope, $state, $q, $mdSidenav, $mdDialog, CaseOntologyDataService, GraphDataService) {
    ...
    vm.toggleSidebar = (componentId) => {
      console.log("toggle sidebar ", componentId);
    ...
    vm.$onInit = () => {
      ...
      Promise.all([
       ...
       // $scope.data.initialCase = angular.copy(result[0][0]);
        ...
        console.log("classTree", vm.classTree );
        $scope.setReady(true);
      }).catch((err) => {
        $scope.setError('SearchAction', 'search', err);
        $scope.setReady(true);
